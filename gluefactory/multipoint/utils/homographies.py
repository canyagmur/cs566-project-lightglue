...
def homographic_adaptation_multispectral(data, net, homographic_adaptation_config={}):
    ...
    if config['aggregation'] == 'window' and config['filter_size'] > 0:
        raise ValueError(
            'Window aggregation assumes keypoints in the binary heatmap!')
    ...
    if config['aggregation'] == 'prod':
        ...
        prob_optical, prob_thermal = search_window(
            out_optical['prob'], out_thermal['prob'], window_size=config["window_size"])  # here
    else:
    ...
    for i in range(1, config['num']):
        ...
        if config['aggregation'] == 'prod':
            ...
            prob_unwarped_optical, prob_unwarped_thermal = search_window(
                unwarped_optical, unwarped_thermal, window_size=config["window_size"])  # here
        else:
            raise ValueError('Unknown aggregation: ' + config['aggregation'])
    ...
    if config['aggregation'] == 'window':
        out_optical = prob_optical / count
        out_thermal = prob_thermal / count
    ...
    if config['min_count'] > 0 and config['aggregation'] != 'window':
        ...
        out_optical[count < config['min_count']] = 0.0  # look here as well
        out_thermal[count < config['min_count']] = 0.0  # look here as well
    ...
    return out, out_optical, out_thermal


def search_window(tensor1, tensor2, window_size):
    ...
    #filtered_x1[filtered_x1 > 0] = 1 # HERE IMPORTANT for weighted keypoint
