...
def compute_descriptor_metrics(net, dataloader, device, config, threshold_keypoints, threshold_warp):
    ...
    for data in tqdm(dataloader):
        ...
        #print(data["hfour_points"])
        #print(out_hm)
        #gt_hm = gt_hm = data["hfour_points"].view(-1,8).float().cpu().numpy().reshape([4,2])
        #out_hm = out_hm.cpu().numpy().reshape([4,2])*32
        #print(gt_hm.shape,out_hm.shape)
        #pts2 = out_hm + gt_hm
        #pred_hm = cv2.getPerspectiveTransform(np.float32(gt_hm),np.float32(pts2))
        #print(pred_hm.shape)
        #print("yeah : ",pred_hm)
        ...
        for (prob_o, prob_t,
            ...
            matching_kp_numbers.append((num_matched_optical + num_matched_thermal) // 2)
